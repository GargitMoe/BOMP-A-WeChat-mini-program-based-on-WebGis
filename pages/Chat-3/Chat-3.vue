...
<template>
	<unicloud-db ref="udb" where="state==true" v-slot:default="{data, loading, error, options}"
		collection="guestbook-3,uni-id-users" field="_id,text,state,user_id._id,user_id.nickname,user_id.avatar_file">
		<view class="chat">
	...
	export default {
		...
		methods: {
			...
			send() {
				...
				const guestbookTable = db.collection('guestbook-3')
